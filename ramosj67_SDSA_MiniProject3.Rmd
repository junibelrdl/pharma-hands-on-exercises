...
wgt_stat<- adsl_eff %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = mean(HEIGHTBL ) %>% round(digits = 1) %>% format(nsmall=1)  ,
            sd = sd(HEIGHTBL ) %>% round(digits = 1) %>% format(nsmall = 1), 
            med = median( HEIGHTBL)  ,              
            min = min(HEIGHTBL ), 
            max = max(HEIGHTBL ),
            n = n())
