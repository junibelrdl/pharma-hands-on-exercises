...
plot1c <- plot1b +
  gghighlight::gghighlight(any(LBNRIND %in% c("HIGH","LOW"))) 
...
plot1d <- plot1b  +
  facet_wrap( ~ TRTP) + 
  gghighlight::gghighlight(any(LBNRIND != "NORMAL"), 
...
```{r}
plot1e <- plot1b +
  labs(x ="Analysis Relative Day"  ,
       y ="Alanine Aminotransferase (U/L)"   )

plot2c <- plot2b +
  labs(x = "Week"  , y = "Alanine Aminotransferase (U/L)"  )
...
p2b <- p2 + plot_annotation(
  title =  "Plot of Alanine Aminotransferase (U/L)",
  subtitle =  ,
...
plot6 <- ALT2 %>%
  filter(WEEK %in% c(0, 1, 2, 4, 8, 16) & LBNRIND %in% c("LOW", "HIGH")) %>%
  geom_point(mapping = aes(x = LBSTRESN, y = as.factor(WEEK)), color = "red")
