...
```{r}
dm_allcomb <- dm_allcomb %>%
  dplyr::mutate(category = factor(category,
                    levels = c("N", "<65", "65-80", ">80", "Mean", "Median","Std Dev","Range(min,max)")))
dm_allcomb %>%
  arrange(category)
```
...
ETHNIC_data_new <- ETHNIC_mrg_cnt3 %>% 
  ...
  mutate(perc_ETHNIC = round((ETHNIC_n/n)*100,1))

ETHNIC_pct <- ETHNIC_data_new %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1)) %>%
  mutate(perc_achar = format(perc_ETHNIC, nsmall = 1))

ETHNIC_n_pct <- ETHNIC_pct %>%
  mutate(npct = paste(ETHNIC_n, paste0("(", perc_achar, ")"))) %>% 
...
RACE_data_new <- RACE_mrg_cnt3 %>% 
  ...
  mutate(perc_RACE = round((RACE_n/n)*100,1))

RACE_pct <- RACE_data_new %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1)) %>%
  mutate(perc_achar = format(perc_RACE, nsmall = 1))
...
```{r}
dm_allcomb1 <- bind_rows(dm_allcomb, RACE_cat, ETHNIC_cat)  

dm_allcomb1 <- dm_allcomb1 %>%
  dplyr::mutate(category = factor(category,
                    levels = c("N", "<65", "65-80", ">80", "Mean", "Median","Std Dev","Range(min,max)","AMERICAN INDIAN OR ALASKA NATIVE","BLACK OR AFRICAN AMERICAN","WHITE","HISPANIC OR LATINO", "NOT HISPANIC OR LATINO")))
